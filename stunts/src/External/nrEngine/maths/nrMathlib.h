...
/**
 * Structure storing two dimensional(x,y) vector with float values 
 * \ingroup math
 */
struct vec2 {
...
/**
 * Structure storing three dimensional(x,y,z) vector with float values 
 * \ingroup math
 */
struct vec3 {
...
/**
 * Structure storing four dimensional (x,y,z,w) vector with float values 
 * \ingroup math
 */
struct vec4 {
...
struct mat4 {
	...
	bool operator==(const mat4 &m){
		for (int i=0; i < 16; i++)if (m[i] != mat[i]) return false;
		return true;
	}
	...
	void copy (const mat4 &mc){
   		for (int i=0; i < 16; i++) mat[i] = mc[i];
    }
	
	void get_row (int row, vec4 & rowv){
		rowv = vec4(mat[0+row],mat[4+row], mat[8+row], mat[12+row]);
	}
	
	vec4 get_rowv (int row){
...
struct quat {
    ...
    void mult_vec( const vec3 &src, vec3 &dst ) const
    {
        ...
        float u_coef = 2.0f * (src.v[0] * x + src.v[1] * y + src.v[2] * z);  
        float c_coef = 2.0f * w;                                       
